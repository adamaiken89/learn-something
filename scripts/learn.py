...
def _list_modules(subject):
    ...
    return sorted(
        ...
        if d.is_dir() and (d / "lesson.md").exists()
    )
...
def cmd_init(args):
    ...
    print(f"Edit syllabus.yaml, then create modules with: learn.py create-module {subject} <module-id>")
...
Commands:
  ...
  epub-verify <subject> [file] Validate EPUB structure
    ...
    p.add_argument('--mermaid', default='api', choices=['api', 'local', 'off'],
