...
learn.sh balance-quiz <topic> <module>      # Re-letter quiz.yaml answers: balanced spread,
                                         # no 3 consecutive same letter. Deterministic per
                                         # topic-module (idempotent). Backup quiz.yaml.bak
learn.sh checksyntax <topic> <module> [--render api|local|off]
                                         # Generation-stage lint: fence balance, mermaid blocks,
                                         # code blocks (python/js/shell via interpreters)
                                         # --render: smoke-render mermaid via render-diagrams
                                         # Exit 1 on hard syntax errors
...
learn.sh validate <topic> [module]           # Full gate: schema + naming + content syntax + quality (see B5)
                                             #   --max-chars N: user-determined lesson.md WARN threshold (default 12,000)
...
├── cumulative_quiz_XX-YY.yaml   # Cross-module quizzes (after every 3-4 modules, span ≤4)
├── modules/
│   ├── NN-name/              # Dir name must match NN-kebab-case
│   │   ├── lesson.md         # Core content + exercises (soft size guide, WARN only; time budget is the cap)
│   │   ├── quiz.yaml         # 8-10 MCQs
...
- Cumulative quizzes: `cumulative_quiz_XX-YY.yaml` (zero-padded range, max span 4 modules, `source_modules` must stay inside `XX-YY`). Single `cumulative_quiz.yaml` rejected.
